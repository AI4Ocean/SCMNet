...
class DatasetProcess:
    ...
    def dataset(self):
        ...
        start_date = datetime.strptime(str(int(np.min(data[:, 2]))), "%Y%m%d").date()
        for i in range(data.shape[0]):
            data[i, 2] = (datetime.strptime(str(int(data[i, 2])), "%Y%m%d").date() - start_date).days
        self.train_data, self.test_data = train_test_split(data, test_size=0.2, random_state=42)

    def save(self):
        data = {'value': self.values.values.tolist(), 'train': self.train_data.tolist(), 'test': self.test_data.tolist()}
...
class ChlDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        y = np.concatenate(
            (self.data[index, 3:303], self.data[index, -5:-2])
        )
